...
<div class="container-fluid">
        ...
        <table class="darkmode table-hover align-middle table-bordered">
            <thead class="table-dark text-center">
                <tr>
                    ...
                    <th scope="col">NOMBRE DE SUBZONA</th>
                    <th scope="col">NOMBRE DE ANTENA</th>
                    <th scope="col">IP</th>
            ...
            <tbody class="text-center">
                ...
                <tr>
                    ...
                    <td>{{ $antena->direccion->zona->nombre_zona ?? 'Sin zona' }}</td>
                    <td>{{ $antena->direccion->nombre_direccion ?? 'Sin subzona' }}</td>
                    <td>{{ $antena->nombre_antena }}</td>
                    <td>{{ $antena->ip }}</td>
                    ...
                    <td>
                        <button type="button" class="btn btn-success btn-sm me-1 mb-1" data-bs-toggle="modal"
                            data-bs-target="#edit{{ $antena->id_antena }}">
                            <i class="bi bi-pencil-square"></i>
                        </button>
                        <button type="button" class="btn btn-danger btn-sm mb-1" data-bs-toggle="modal"
                            data-bs-target="#delete{{ $antena->id_antena }}">
                            <i class="bi bi-trash"></i>
                        </button>
                    </td>

                </tr>
                @include('antenas.edite')
                @include('antenas.delete')
                @empty
                <tr>
    ...
    <div class="d-block d-md-none">
        ...
        <div class="card mb-3 bg-dark text-white border-secondary">
            <div class="card-header d-flex justify-content-between align-items-center py-2" data-bs-toggle="collapse"
                data-bs-target="#collapseDireccion{{$antena->id_antena}}" aria-expanded="false" style="cursor: pointer">
                <h5 class="mb-0">{{$antena->nombre_antena}}</h5>
                <i class="bi bi-chevron-down transition-all"></i>
            </div>

            <div class="collapse" id="collapseDireccion{{$antena->id_antena}}">
                <div class="card-body pt-2">
                    <div class="row g-2 mb-2">
                        <div class="col-6">
                            <small class="text-muted">ID</small>
                            <p class="mb-0">{{$antena->id_antena}}</p>
                        </div>
                        <div class="col-6">
                            <small class="text-muted">NOMBRE DE ZONA</small>
                            <p class="mb-0">{{ $antena->direccion->zona->nombre_zona ?? 'Sin zona' }}</p>
                        </div>
                        <div class="col-6">
                            <small class="text-muted">NOMBRE DE SUBZONA</small>
                            <p class="mb-0">{{ $antena->direccion->nombre_direccion ?? 'Sin subzona' }}</p>
                        </div>
                        <div class="col-6">
                            <small class="text-muted">NOMBRE DE ANTENA</small>
                            <p class="mb-0">{{ $antena->nombre_antena ?? 'No asignado' }}</p>
                        </div>
                        <div class="col-6">
                            <small class="text-muted">IP</small>
                            <p class="mb-0">{{ $antena->ip ?? 'No asignado' }}</p>
                        </div>
                    </div>
    ...
    <div class="flex justify-content-between align-items-center mt-3">
        <div>
            {{ $antenas->links('pagination::bootstrap-5') }}
        </div>
    </div>
...
<!-- Incluir modales -->
@include('antenas.create')

@foreach ($antenas as $antena)
@include('antenas.edite', ['antena' => $antena])
@include('antenas.delete', ['antena' => $antena])
@endforeach
